...
## 🧾 Bundled Sample Invoices

The `sample_invoices/` folder contains **27 real invoice PDFs** (K-series and MUM-series GST invoices).
You can try them directly from the UI without uploading anything:

1. On the Home page, click **"Sample Invoices (27 bundled)"** to expand the panel.
2. Click ▶ next to any invoice to extract it individually.
3. Click **"Extract All"** to run batch extraction across all 27 invoices and view results in History.

### New API Endpoints (added with sample invoices)

| Method | Path | Description |
|---|---|---|
| GET | `/sample-invoices` | List all bundled sample invoices |
| POST | `/extract-sample?filename=X` | Extract one sample invoice by name |
| POST | `/extract-all-samples` | Extract all 27 bundled sample invoices |
| GET | `/sample-invoices/file/{filename}` | Serve a sample PDF for preview |
